...
int main(int argc, char ** argv){
    ...
    auto node = std::make_shared<rclcpp::Node>("add2ints_client_no_oop");

    auto client = node->create_client<example_interfaces::srv::AddTwoInts>("add_two_ints_service");
    ...
    if (rclcpp::spin_until_future_complete(node, future) == rclcpp::executor::FutureReturnCode::SUCCESS){
        ...
        RCLCPP_INFO(node->get_logger(), "Error while calling service server..");
    }
